...
import { AlertTriangle, Lock, Info, ChevronDown, ChevronUp, Ban } from 'lucide-react';
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {/* Always show weight when it differs from current */}
                      {alt.adjustedWeight !== currentWeight && (
                        <span className={cn(
                          'text-xs font-medium',
                          alt.adjustedWeight < currentWeight ? 'text-amber-600' : 'text-green-600'
                        )}>
                          → {formatWeight(alt.adjustedWeight)}
                          {alt.coefficientDiffers && (
                            <span className="text-muted-foreground ml-1">(коэф.)</span>
                          )}
                        </span>
                      )}
                      {alt.adjustedWeight === currentWeight && alt.tag === 'equal' && (
